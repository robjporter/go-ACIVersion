...
func (a *Application) runAll() {
	...
	if len(a.ACI) > 0 {
		for i := 0; i < len(a.ACI); i++ {
			a.LogInfo("Attempting to connect to ACI APIC", map[string]interface{}{"System": a.ACI[i].ip}, false)
			myaci := aci.New()
			myaci.Login(a.ACI[i].ip, a.ACI[i].username, a.DecryptPassword(a.ACI[i].password))
			if myaci.LastResponse.Errors == nil {
				a.ACI[i].version = myaci.GetVersion()
				a.ACI[i].status = true
				a.LogInfo("Successfully connected to ACI APIC", map[string]interface{}{"System": a.ACI[i].ip}, false)
			} else {
				a.ACI[i].status = false
				a.LogInfo("Failed to connect to ACI APIC", map[string]interface{}{"System": a.ACI[i].ip}, false)
			}
			myaci.Logout()
		}
	} else {
		fmt.Println("No ACI APICs detected in the config file.  Please trying adding one and try again.")
	}
	a.processVersions()
...
func (a *Application) outputVersionSuggestions() {
	a.LogInfo("Displaying ACI APIC Version information", nil, false)
	table := terminal.New([]string{"ACI APIC", "Current Version", "Is Deferred", "Suggested Version", "Latest Train Version", "Latest Version"})
	for i := 0; i < len(a.ACI); i++ {
		if a.ACI[i].status != false {
			table.AddRow(map[string]interface{}{"ACI APIC": a.ACI[i].ip, "Current Version": a.ACI[i].version, "Is Deferred": a.ACI[i].deferredVersion, "Suggested Version": a.ACI[i].suggestedVersion, "Latest Version": a.ACI[i].releaseVersion, "Latest Train Version": a.ACI[i].releaseTrainVersion})
		}
	}
	table.Print()
}
...
func (a *Application) processVersions() {
	a.LogInfo("Getting ACI APIC Version information from Cisco", nil, false)
	aci.GetWebData()
	a.LogInfo("Processing discovered ACI APIC versions", nil, false)
	for i := 0; i < len(a.ACI); i++ {
		if a.ACI[i].status != false {
			a.LogInfo("Validating ACI APIC Version Information", map[string]interface{}{"Version": a.ACI[i].version}, false)
			a.ACI[i].suggestedVersion = aci.GetSuggestedReleaseTrain(strings.TrimSpace(a.ACI[i].version))
			a.LogInfo("Validating if deferred Version", map[string]interface{}{"Version": a.ACI[i].version}, false)
			a.ACI[i].deferredVersion = aci.GetIsDeferredRelease(strings.TrimSpace(a.ACI[i].version))
			a.LogInfo("Validating latest train Version", map[string]interface{}{"Version": a.ACI[i].version}, false)
			a.ACI[i].releaseVersion = aci.GetLatestRelease()
			a.LogInfo("Validating latest Version", map[string]interface{}{"Version": a.ACI[i].version}, false)
			a.ACI[i].releaseTrainVersion = aci.GetLatestReleaseTrain(strings.TrimSpace(a.ACI[i].version))
		}
	}
}
...
func (a *Application) displayBanner() {
	...
	banner.PrintNewFigure("ACI Version", "rounded", true)
	fmt.Println(colors.Color("Cisco Application Centric Infrastructure Version checking tool v"+a.Version, colors.BRIGHTYELLOW))
...
func (a *Application) saveConfig() {
	...
	if len(a.ACI) > 0 {
		...
		a.Config.Set("aci.systems", items)
	}
...
func (a *Application) processSystems() []interface{} {
	...
	for i := 0; i < len(a.ACI); i++ {
		...
		item["url"] = a.ACI[i].ip
		item["username"] = a.ACI[i].username
		item["password"] = a.ACI[i].password
